test_that("each row is one individual, all individuals are included", {
  expect_identical(nrow(anim_timespan(wolf_samples$AnimalRef, wolf_samples$Date, wolf_samples$SType == "Tissue")),
...
test_that("FirstSeen and LastSeen are min and max sample dates of particular individual", {
  ...
  expect_identical(suppressMessages(anim_timespan(subset$AnimalRef, subset$Date, subset$SType == "Tissue"))[c(2,3)],
                   data.frame(FirstSeen = min(subset$Date),LastSeen = max(subset$Date)))
})

test_that("No duplicated individuals", {
  result <- anim_timespan(wolf_samples$AnimalRef, wolf_samples$Date, wolf_samples$SType == "Tissue")
