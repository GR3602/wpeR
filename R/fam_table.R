...
#'
#' @description `PackTable` combines pedigree and sample data for visual ([`PedigreeCMRSatplot`])
...
#'       (default = `FirstSeen` and `LastSeen`, see [`make.animal.timespan`] function),
#'   - value identifying if if the individual is dead; logical
#'       (default = `IsDead`, see [`make.animal.timespan`] function).
#'
#'
#'
#' @param packs data frame. (Subset of) family (`packs`) data generated by [`organizePacks`] function.
#'   Just families that we want to plot have to be selected.
#' @param all.packs data frame. Family (`packs`) data generated by [`organizePacks`] function.
#' @param ped data frame. Organized pedigree (`ped`) generated by [`organizePacks`] function.
...
#' For description of `sampledata` structure and sample information needed for `PackTable()` see Details.
...
#'   * `PackID`: numeric. Identifier number of family that individual belongs to,
...
#'    (current family defined with `PackID` for a particular entry),
...
#'
#'
#' PackTable(org_tables$packs,
#'           org_tables$packs,
#'           org_tables$ped,
#'           sampledata,
#'           deadSample = c("Tissue", "Decomposing Tissue", "Blood"))
#'
PackTable<- function(packs, all.packs, ped, sampledata,
  ...
  #packs = packs data (from 'organizePacks' function), selected only packs we want to plot.
  ...
  outdata = NULL
  plottingID = 1 #ID for plotting, since the same animal can be drawn in more packs in polygamy

  ##loop fills the outdata table with data from sampledata[datacolumn,], adds packID, polycluster and creates plottingID, alpha (if animal is reporductive), later_alpha (if animal repoductive in later season), is Polyamous (if it has offsping with more than one other animal)
  ...
  for (i in 1:nrow(packs)) {
    ...
    if(!grepl("//*", packs$father[i])) {

      fatherSamples = sampledata[sampledata$AnimalRef == packs$father[i], datacolumns]

      fatherSamples$plottingID = rep(plottingID, nrow(fatherSamples))
      plottingID = plottingID +1

      fatherSamples$PackID = rep(packs$PackID[i], nrow(fatherSamples))
      fatherSamples$polyCluster = rep(packs$polyCluster[i], nrow(fatherSamples))
      ...
      if(!is.na(packs$DadPclust[i])) fatherSamples$isPolygamous = rep(TRUE, nrow(fatherSamples))
    ...
    if(!grepl("#", packs$mother[i])) {

      motherSamples = sampledata[sampledata$AnimalRef == packs$mother[i], datacolumns]

      motherSamples$plottingID = rep(plottingID, nrow(motherSamples))
      plottingID = plottingID +1

      motherSamples$PackID = rep(packs$PackID[i], nrow(motherSamples))
      motherSamples$polyCluster = rep(packs$polyCluster[i], nrow(motherSamples))
      ...
      if(!is.na(packs$MomPclust[i])) motherSamples$isPolygamous = rep(TRUE, nrow(motherSamples))
    ...
    sub_ped = ped[ped$PackID == packs$PackID[i],]
    ...
    for(j in 1:nrow(sub_ped)) {
      ...
      offspringSamples$PackID = rep(packs$PackID[i], nrow(offspringSamples))
      offspringSamples$polyCluster = rep(packs$polyCluster[i], nrow(offspringSamples))
      offspringSamples$alpha = rep(FALSE, nrow(offspringSamples))

      if (sum(grepl(sub_ped$id[j], all.packs$father) | grepl(sub_ped$id[j], all.packs$mother)) > 0)
