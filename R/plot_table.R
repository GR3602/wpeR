...
#'
#'
#' @param plot.fams Character string or numeric vector. FamID numbers from `fams` data generated by [`org_fams()`] function.
...
#'
plot_table <- function(plot.fams = "all", all.fams, ped, sampledata,
                       datacolumns = c("Sample", "AnimalRef", "GeneticSex",
                                       "Date", "SType", "lat", "lng",
                                       "FirstSeen", "LastSeen", "IsDead"),
                       deadSample = c("Tissue", "Decomposing Tissue", "Bone")) {

  if (all(plot.fams == "all")) {
    sel.fams <- all.fams
  } else if (all(plot.fams %in% all.fams$FamID)) {
    sel.fams <- all.fams[which(all.fams$FamID %in% plot.fams),]
  } else {
    stop("FamID/s ", plot.fams[!(plot.fams %in% all.fams$FamID)], " not present in all.fams data frame")
  }
