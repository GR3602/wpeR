...
2.  **Genetic samples metadata**.<br /> This dataset should include information on all genetic samples belonging to the animals included in the pedigree and must include columns that describe:
    ...
    -   Sample type (eg: scat, urine, tissue) the sample type labels are used by downstream functions to infer whether an animal was alive or dead at the time of collection.<br />
...
By using the dead argument, users specify which sample types (e.g., "Tissue" from a carcass) indicate a mortality event. An individual is then recorded as "dead" at the point that specific sample type was collected.
