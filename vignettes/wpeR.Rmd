...
library(ggplot2)
library(basemaps)
#library(ggsflabel) not a CRAN package solved bellow
library(gridExtra)
library(sf)
...
#since ggsflabel is not CRAN package the labels are added seperatly
if (requireNamespace("ggsflabel")) {
  library(ggsflabel)
  
  ped.ref <- ped.ref+
    geom_sf_label_repel(data = ps.t$motherRpoints, aes(label = AnimalRef ), size = 2, , fill = "#b7484b", alpha = 0.6)+
    geom_sf_label_repel(data = ps.t$fatherRpoints, aes(label = AnimalRef ), size = 2, , fill = "#1f78b4", alpha = 0.6)+
    geom_sf_label_repel(data = ps.t$offspringRpoints, aes(label = AnimalRef ), size = 1.5, fill = "#ff7f00", alpha = 0.6)

  
  ped.ref2 <- ped.ref2 +
    geom_sf_label_repel(data = ps.tl.t$motherRpoints, aes(label = AnimalRef ), size = 2, , fill = "#b7484b", alpha = 0.6)+
    geom_sf_label_repel(data = ps.tl.t$fatherRpoints, aes(label = AnimalRef ), size = 2, , fill = "#1f78b4", alpha = 0.6)+
    geom_sf_label_repel(data = ps.tl.t$offspringRpoints, aes(label = AnimalRef ), size = 1.5, fill = "#ff7f00", alpha = 0.6)
  
  par.mov <- par.mov+
    geom_sf_label_repel(data = ps.t$motherRpoints, aes(label = AnimalRef ), size = 2, , fill = "#b7484b", alpha = 0.6)+
    geom_sf_label_repel(data = ps.t$fatherRpoints, aes(label = AnimalRef ), size = 2, , fill = "#1f78b4", alpha = 0.6)+
    geom_sf_text_repel(data = ps.t$motherMovePoints, aes(label = as.character(Date)), size = 1.5, alpha = 0.4)+
    geom_sf_text_repel(data = ps.t$fatherMovePoints, aes(label = as.character(Date)), size = 1.5, alpha = 0.4)

  
  par.mov2 <- par.mov2+
    geom_sf_label_repel(data = ps.tl.t$motherRpoints, aes(label = AnimalRef ), size = 2, , fill = "#b7484b", alpha = 0.6)+
    geom_sf_label_repel(data = ps.tl.t$fatherRpoints, aes(label = AnimalRef ), size = 2, , fill = "#1f78b4", alpha = 0.6)+
    geom_sf_text_repel(data = ps.tl.t$fatherMovePoints, aes(label = as.character(Date)), size = 1.5, alpha = 0.4)+
    geom_sf_text_repel(data = ps.tl.t$motherMovePoints, aes(label = as.character(Date)), size = 1.5, alpha = 0.4)


  
  offs.mov <- offs.mov+
    geom_sf_text_repel(data = ps.t$offspringMovePoints, aes(label = as.character(Date)), size = 1.5, alpha = 0.4)+
    geom_sf_label_repel(data = ps.t$offspringRpoints, aes(label = AnimalRef ), size = 1.5, fill = "#ff7f00", alpha = 0.6)+
    geom_sf_label_repel(data = ps.t$offspringMovePoints, aes(label = AnimalRef ), size = 1.5, fill = "white", alpha = 0.3)
  
  offs.mov2 <- offs.mov2+
    geom_sf_text_repel(data = ps.tl.t$offspringMovePoints, aes(label = as.character(Date)), size = 1.5, alpha = 0.4)+
    geom_sf_label_repel(data = ps.tl.t$offspringRpoints, aes(label = AnimalRef ), size = 1.5, fill = "#ff7f00", alpha = 0.6)+
    geom_sf_label_repel(data = ps.tl.t$offspringMovePoints, aes(label = AnimalRef ), size = 1.5, fill = "white", alpha = 0.3)

}else{
  message("Package 'ggsflabel' is not installed. If you want to display the labels with as little work as posiible install it from GitHub: devtools::install_github('yutannihilation/ggsflabel')")
}
